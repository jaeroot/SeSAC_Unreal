...
{
	// // 동기화된 방식, UParticleSystem : 캐스케이드 파티클 시스템 에셋 타입
	// // LoadObject : 참조 없는 애셋을 로딩하고 참조할 때 사용
	// auto Particle = LoadObject<UParticleSystem>(GetWorld(), TEXT(""));
	//
	...
	// spawnemitteratlocation : 위치에 소환, spawnemitterattach : 부착한 위치에 소환
	// impact normal : 방향 벡터, rotation으로 바꿀 수 있음
	// auto destroy : 파티클 끝나면 월드에서 제거해줌
	
	// FindObject : 참조 있는 애셋을 참조할 때 사용
	// FindObject<UParticleSystem>(GetWorld(), TEXT(""));
